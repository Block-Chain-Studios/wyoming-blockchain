...
class Browse extends CI_Controller {
	...
	function filter($type = '', $genre_id = '', $actor_id = '', $director_id ='', $year = '')
	{
		if (empty($type) || empty($genre_id) || empty($actor_id)) {
			redirect(base_url().'index.php?browse/home' , 'refresh');
		}
		if ($actor_id == 'all' && $director_id == 'all' && $year == 'all' && $type == 'movie') {
			redirect(base_url().'index.php?browse/movie/'.$genre_id , 'refresh');
		}else if ($actor_id == 'all' && $director_id == 'all' && $year == 'all' && $type == 'series') {
		...
		$page_data['items'] = $this->crud_model->get_actor_genre_wise_movies_and_tv_series($actor_id, $type, $genre_id, $director_id, $year);
